...
<%--  
	...
	Description: 综合查询-当期数据-企业评级信息记录-基础段信息表详情页面
--%>
<%
	...
	if(busiIdSerialNo != null && !PIDRConstants.VALUE_NULL.equals(busiIdSerialNo)) {
		map = ValidateErrors.getErrInfo(PIDRConstants.TABLE_PIDR_RATE_BS, busiIdSerialNo, Sqlca);
		...
		map = ValidateErrors.getNAErrInfo(PIDRConstants.TABLE_PIDR_RATE_BS, alertErrorKey, Sqlca);
		for(String key : map.keySet()) {//提示信息英文名不用展示
		    errMsg += "错误字段"+map.get(key).substring(map.get(key).indexOf(":"))+"<br/>";
		}
	}
...
%>
<%	
	ASObjectModel doTemp = new ASObjectModel("PidrRateBsInfo");
...
<script type="text/javascript">
	...
	function saveRecord(){
		...
		if('<%=operationType%>' == '<%=PIDRConstants.OPERATIONTYPE_ADD%>'){//新增操作打开页面
			var flag = checkPrimaryKey("PIDR_RATE_BS","CustomerId");//判断新增记录是否已存在
			if(flag){
	...
	function handle(){
		...
		if('<%=operationType%>' == '<%=PIDRConstants.OPERATIONTYPE_ADD%>'){//新增
			//setTextReadOnly(0,0,"CustomerId",true);//新增保存之后，主键置为只读
			...
			AsControl.OpenView('/PIDR/IntegatedQueryManage/CurrentDataManage/RateManage/PidrRateBs/PidrRateBsInfo.jsp',
					params,"","");
		}else{//详情的保存
			var retValue = RunJavaMethodTrans("com.amarsoft.app.pidr.integatedquery.PIDRRateUpdateHandler", "reverseUpdate", "CustomerId=" + customerId +",TableName=PIDR_RATE_BS");
			if("success" != retValue){
				alert("反向更新记录标志失败！");
			}
			reloadSelf();
	...
	function initRow() {
		if(getRowCount(0) == 0) {//新增
			...
			setItemValue(0,getRow(),"InfRecType","<%=PIDRConstants.NORMALREPORT_RATE%>"); 
