...
<%--  
	...
	Description: 综合查询-当期数据-企业评级信息记录-主要负责人段信息表详情页面
--%>
<%
	...
	if(busiIdSerialNo != null && !PIDRConstants.VALUE_NULL.equals(busiIdSerialNo)) {
		map = ValidateErrors.getErrInfo(PIDRConstants.TABLE_PIDR_RATE_LEGREPINF, busiIdSerialNo, Sqlca);
		...
		map = ValidateErrors.getNAErrInfo(PIDRConstants.TABLE_PIDR_RATE_LEGREPINF, alertErrorKey, Sqlca);
		for(String key : map.keySet()) {//提示信息英文名不用展示
		    errMsg += "错误字段"+map.get(key).substring(map.get(key).indexOf(":"))+"<br/>";
		}
	}
...
%>
<%	
	ASObjectModel doTemp = new ASObjectModel("PidrRateLegrepinfInfo");
...
<script type="text/javascript">
	...
	function saveRecord(){
		...
		if('<%=operationType%>' == '<%=PIDRConstants.OPERATIONTYPE_ADD%>'){//新增操作打开页面
			var flag = checkPrimaryKey("PIDR_RATE_LEGREPINF","CustomerId");//判断新增记录是否已存在
			if(flag){
	...
	function handle(){
		...
		if('<%=operationType%>' == '<%=PIDRConstants.OPERATIONTYPE_ADD%>'){//新增
			alert("新增时\n请确保除对外投资信息段、负面信息段外，其余信息段均有对应记录\n若存在对外投资信息、负面信息，则在对应信息段新增，不存在可不新增");
			...
			AsControl.OpenView('/PIDR/IntegatedQueryManage/CurrentDataManage/RateManage/PidrRateLegrepinf/PidrRateLegrepinfInfo.jsp',
					params,"","");
		}else{//详情的保存
			var retValue = RunJavaMethodTrans("com.amarsoft.app.pidr.integatedquery.PIDRRateUpdateHandler", "reverseUpdate", "CustomerId=" + customerId +",TableName=PIDR_RATE_LEGREPINF");
			if("success" != retValue){
				alert("反向更新记录标志失败！");
			}
			reloadSelf();
