...
--%>
 <%
    ASObjectModel doTemp = new ASObjectModel("PidrRateCashflowList");
...
<script type="text/javascript">
	...
	function add(){
		...
		AsDialog.PopView(url,'TableName=PIDR_RATE_CASHFLOW','dialogWidth:500px;dialogHeight:300px',function(){
			reloadSelf();
		});
	}
	...
	function del(){
		...
		layer.confirm("删除",{
			...
			if(recordFlag != '<%=PIDRConstants.RECORDFLAG_A%>' ){
				...
				var retValue = RunJavaMethodTrans("com.amarsoft.app.pidr.alldlt.PidrCascadeDelete", "deleteRateById", "customerId=" + customerId +",tableName="+'<%=PIDRConstants.TABLE_PIDR_RATE_CASHFLOW%>');
				alert(retValue);
				layer.close(index);
				reloadSelf();	
			}
    ...
    function viewRecord(){
         var url = "/PIDR/IntegatedQueryManage/CurrentDataManage/RateManage/PidrRateCashflow/PidrRateCashflowInfoFrame.jsp";
         ...
         AsDialog.PopView(url,'CustomerId=' + customerId +'&FinstaYear=' + finstaYear +'&SheetType=' + sheetType +'&SheetTypeDivide=' + sheetTypeDivide +'&OperationType=<%=PIDRConstants.OPERATIONTYPE_INFO%>',{title:finstaYear+"年度现金流量表"},function(){
        	 reloadSelf();
         });
    }
    /*~[Describe=查看历史记录,InputParam=无,OutputParam=无]~*/
    function seeHis(){
        var url = "/PIDR/IntegatedQueryManage/HistoryDataManage/RateManage/HPidrRateCashflow/HPidrRateCashflowList.jsp";
        var customerId = getItemValue(0,getRow(0),'CustomerId');
        if(typeof(customerId)=="undefined" || customerId.length==0 ){
               alert("请选择一条记录");
               return ;
            }
        AsDialog.PopView(url,'Flag=2&CustomerId='+customerId,'dialogWidth:900px;dialogHeight:600px');
   }
    ...
    function mdfcInfo(){
		...
		var retValue=RunJavaMethodTrans("com.amarsoft.app.pidr.mdfc.ModifyRateFinanceInfo","modifyRateInfo",
				"CustomerId=" + customerId + ",MdfcSgmtCode=I, ManagerOrgId=" + managerOrgId 
		...
		var params = 'SessionId=<%=PIDRConstants.SESSIONID_14%>&CustomerId=' + customerId + '&MdfcSgmtCode=I' ;
