...
<%--  
	...
	Description: 综合查询-当期数据-企业评级信息记录-对外投资段信息表详情页面
--%>
<%
	...
	if(busiIdSerialNo != null && !PIDRConstants.VALUE_NULL.equals(busiIdSerialNo)) {
		map = ValidateErrors.getErrInfo(PIDRConstants.TABLE_PIDR_RATE_INVABROINF, busiIdSerialNo, Sqlca);
		...
		map = ValidateErrors.getNAErrInfo(PIDRConstants.TABLE_PIDR_RATE_INVABROINF, alertErrorKey, Sqlca);
		for(String key : map.keySet()) {//提示信息英文名不用展示
		    errMsg += "错误字段"+map.get(key).substring(map.get(key).indexOf(":"))+"<br/>";
		}
	}
...
%>
<%	
	ASObjectModel doTemp = new ASObjectModel("PidrRateInvabroinfInfo");
...
<script type="text/javascript">
    ...
    var invEntEstablishDate = getItemValue(0,getRow(0),'InvEntEstablishDate');//被投资企业成立日期字段的值（isUserChanged()无法判断使用内置按钮选择日期方法下值的变化）
    var invDate = getItemValue(0,getRow(0),'InvDate');//投资日期字段的值（isUserChanged()无法判断使用内置按钮选择日期方法下值的变化）
	...
	function saveRecord(){
		...
		if('<%=operationType%>' == '<%=PIDRConstants.OPERATIONTYPE_ADD%>'){//新增操作打开页面
			var flag = checkPrimaryKey("PIDR_RATE_INVABROINF","CustomerId@RelaId");//判断新增记录是否已存在
			if(flag){
		...
		if(isUserChanged() || getItemValue(0,getRow(0),'CustomerId') != oldCustomerId || getItemValue(0,getRow(0),'InvEntEstablishDate') != invEntEstablishDate || getItemValue(0,getRow(0),'InvDate') != invDate){//页面是否发生变化
			if(confirm("确定保存吗？")){
	...
	function handle(){
		...
		if('<%=operationType%>' == '<%=PIDRConstants.OPERATIONTYPE_ADD%>'){//新增
		    ...
		    //若第一次新增时无对外投资信息，后续新增对外投资时，需要反向更新基础段未更新记录
			var retValue = RunJavaMethodTrans("com.amarsoft.app.pidr.integatedquery.PIDRRateUpdateHandler", "reverseUpdate", "CustomerId=" + customerId +",TableName=PIDR_RATE_INVABROINF");
			...
			if("success" == retValue){
				...
				AsControl.OpenView('/PIDR/IntegatedQueryManage/CurrentDataManage/RateManage/PidrRateInvabroinf/PidrRateInvabroinfInfo.jsp',
						params,"","");
			}else{
				alert("反向更新记录标志失败！");
			}
