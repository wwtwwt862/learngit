...
<%--  
	...
	Description: 综合查询-当期数据-企业评级信息记录-注册资本信息表列表页面
--%>
<%	
	ASObjectModel doTemp = new ASObjectModel("PidrRateRegcapList");
...
<script type="text/javascript">
	...
	function add(){
		var url = "/PIDR/IntegatedQueryManage/CurrentDataManage/RateManage/PidrRateRegcap/PidrRateRegcapInfo.jsp";
	...
	function del(){
		...
		layer.confirm("删除",{
			...
			if(recordFlag != '<%=PIDRConstants.RECORDFLAG_A%>' ){
				...
				var retValue = RunJavaMethodTrans("com.amarsoft.app.pidr.alldlt.PidrCascadeDelete", "deleteRateById", "customerId=" + customerId +",tableName="+'<%=PIDRConstants.TABLE_PIDR_RATE_REGCAP%>');
				alert(retValue);
				layer.close(index);
				reloadSelf();	
			}
	...
	function seeHis(){
		var url = "/PIDR/IntegatedQueryManage/HistoryDataManage/RateManage/HPidrRateRegcap/HPidrRateRegcapList.jsp";
		var customerId = getItemValue(0,getRow(0),'CustomerId');// 客户编号
