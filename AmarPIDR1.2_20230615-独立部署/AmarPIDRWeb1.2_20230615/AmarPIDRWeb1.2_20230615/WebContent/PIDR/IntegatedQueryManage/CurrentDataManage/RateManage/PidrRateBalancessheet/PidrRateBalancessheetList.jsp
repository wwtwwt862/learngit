...
<%@ include file="/Frame/resources/include/include_begin_list.jspf"%>
 ...
 <%--  
    ...
    Description:企业评级信息  当期数据    资产负债表   列表页面
--%>
 <%
    ASObjectModel doTemp = new ASObjectModel("PidrRateBalancessheetList");
 ...
 */  
     String sButtons[][] = {
             ...
             {"true","","Button","查看历史记录","查看历史记录","seeHis()","","","","btn_icon_check",""}, 
             {"true","","Button","按段更正","按段更正","mdfcInfo()","","","","btn_icon_detail",""},
     };
...
<script type="text/javascript">
	...
	function add(){
		...
		AsDialog.PopView(url,'TableName=PIDR_RATE_BALANCESSHEET','dialogWidth:500px;dialogHeight:300px',function(){
			reloadSelf();
		});
	}
	...
	function del(){
		...
		layer.confirm("删除",{
			...
			if(recordFlag != '<%=PIDRConstants.RECORDFLAG_A%>' ){
				...
				var retValue = RunJavaMethodTrans("com.amarsoft.app.pidr.alldlt.PidrCascadeDelete", "deleteRateById", "customerId=" + customerId +",tableName="+'<%=PIDRConstants.TABLE_PIDR_RATE_BALANCESSHEET%>');
				alert(retValue);
				layer.close(index);
				reloadSelf();	
			}
    ...
    function viewRecord(){
         var url = "/PIDR/IntegatedQueryManage/CurrentDataManage/RateManage/PidrRateBalancessheet/PidrRateBalancessheetInfoFrame.jsp";
        ...
        AsDialog.PopView(url,'CustomerId=' + customerId +'&FinstaYear=' + finstaYear +'&SheetType=' + sheetType +'&SheetTypeDivide=' + sheetTypeDivide +'&OperationType=<%=PIDRConstants.OPERATIONTYPE_INFO%>',{title:finstaYear+"年度资产负债表"},function (){
            reloadSelf();
        });
    }
    /*~[Describe=查看历史记录,InputParam=无,OutputParam=无]~*/
    function seeHis(){
        var url = "/PIDR/IntegatedQueryManage/HistoryDataManage/RateManage/HPidrRateBalancessheet/HPidrRateBalancessheetList.jsp";
    ...
    function mdfcInfo(){
        ...
        var retValue=RunJavaMethodTrans("com.amarsoft.app.pidr.mdfc.ModifyRateFinanceInfo","modifyRateInfo",
                "CustomerId=" + customerId + ",MdfcSgmtCode=G, ManagerOrgId=" + managerOrgId 
        ...
        var params = 'SessionId=<%=PIDRConstants.SESSIONID_14%>&CustomerId=' + customerId + '&MdfcSgmtCode=G' ;
