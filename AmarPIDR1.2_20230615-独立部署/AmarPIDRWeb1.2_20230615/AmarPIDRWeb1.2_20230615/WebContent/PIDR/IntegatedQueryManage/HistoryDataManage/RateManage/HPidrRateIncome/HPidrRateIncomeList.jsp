...
<%@ include file="/Frame/resources/include/include_begin_list.jspf"%>
 ...
 <%--  
    ...
    Description:企业评级信息  历史数据    利润表   列表页面
--%>
 <%
    ASObjectModel doTemp = new ASObjectModel("HPidrRateIncomeList");
	...
	if(PIDRConstants.MDFC_FLAG.equals(flag) || PIDRConstants.FEEDBACK_FLAG.equals(flag)) {
		doTemp.appendJboWhere("CustomerId =:CustomerId AND SessionId =:SessionId ");  // 按段更正详情中待更正历史段列表打开控制显示
	}else if( PIDRConstants.FEEDBACK_FLAG.equals(flag)){
		doTemp.appendJboWhere("CustomerId =:CustomerId AND SessionId =:SessionId ");  // 反馈错误打开控制显示
	}
...
/*~[Describe=详情,InputParam=无,OutputParam=无]~*/
    function viewRecord(){
			var url = "/PIDR/IntegatedQueryManage/HistoryDataManage/RateManage/HPidrRateIncome/HPidrRateIncomeInfoFrame.jsp";
			...
			AsDialog.PopView(url,'CustomerId=' + customerId +'&FinstaYear=' + finstaYear +'&SheetType=' + sheetType +'&SheetTypeDivide=' + sheetTypeDivide +'&SessionId=' + sessionId +'&fNo='+fNo+'&MessageType='+messageType+ '&RightType=<%=rightType%>&Flag=<%=flag%>',{title:finstaYear+"年度利润表"},function(){
			 reloadSelf();
});
    }
</script>
