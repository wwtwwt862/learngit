...
<%@ page import="com.amarsoft.app.pidr.common.PIDRConstants"%>
 <%--  
    ...
    Description:企业评级信息  历史数据    资产负债表   列表页面
--%>
 <%
    ASObjectModel doTemp = new ASObjectModel("HPidrRateBalancessheetList");
	...
	if(PIDRConstants.MDFC_FLAG.equals(flag)) {
		argsValue = sessionId + "," + customerId;
...
/*~[Describe=详情,InputParam=无,OutputParam=无]~*/
    function viewRecord(){
			var url = "/PIDR/IntegatedQueryManage/HistoryDataManage/RateManage/HPidrRateBalancessheet/HPidrRateBalancessheetInfoFrame.jsp";
			...
			AsDialog.PopView(url,'CustomerId=' + customerId +'&FinstaYear=' + finstaYear +'&SheetType=' + sheetType +'&SheetTypeDivide=' + sheetTypeDivide +'&SessionId=' + sessionId + '&RightType=<%=rightType%>&Flag=<%=flag%>&fNo='+fNo+'&MessageType='+messageType,{title:finstaYear+"年度资产负债表"},function(){
			 reloadSelf();
});
    }
</script>
