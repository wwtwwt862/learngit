...
<%--  
	...
	Description: 综合查询-历史数据-企业评级信息记录-负面信息表列表页面
--%>
<%	
	ASObjectModel doTemp = new ASObjectModel("PidrRateNeginfList");
	doTemp.setJboClass("jbo.pidr.HPIDR_RATE_NEGINF");// 设置JBO对象
...
<script type="text/javascript">
	...
	function view(){
		var url = "/PIDR/IntegatedQueryManage/HistoryDataManage/RateManage/HPidrRateNeginf/HPidrRateNeginfInfo.jsp";
		var sessionId = getItemValue(0,getRow(0),'SessionId');// 期次号
		var relaId = getItemValue(0,getRow(0),'RelaId');// 客户编号
		var customerId = getItemValue(0,getRow(0),'CustomerId');// 客户编号
		var rightType = "<%=rightType%>";
		var fNo = "<%=fNo%>";
		var messageType = "<%=messageType%>";
		var params = 'SessionId=' + sessionId + '&RelaId=' + relaId + '&CustomerId=' + customerId + '&RightType=' + rightType +'&fNo='+fNo+'&MessageType='+messageType+ '&Flag=<%=flag%>';
