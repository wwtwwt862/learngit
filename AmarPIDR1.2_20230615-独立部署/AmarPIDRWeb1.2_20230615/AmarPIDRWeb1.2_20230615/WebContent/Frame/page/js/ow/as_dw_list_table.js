...
TableBuilder.displayColor = function(obj, color){
	for(var i = 0; i < sWDColors.length; i++){
		$(obj).removeClass(sWDColors[i]);
	}
	if(color) $(obj).addClass(color);
};
...
TableBuilder.getFomatedNumber = function(str,checkFormat){
	...
	}else if(checkFormat==7){
		str = FormatKNumber(str,TableFactory.WanYuanScale.length);
...
TableBuilder.prototype.createTableHTML = function(){
	...
	sbResult.append('</div>');
	//创建冻结数据列
	sbResult.append('<div id="'+ this.tableId +'_static" onscroll="this.scrollLeft=0;" style="height:'+ this.floatHeight +'px;width:0px;overflow:hidden;">');
	sbResult.append('<div style="overflow-x:scroll;overflow-y:auto;width:3000px;height:100%;" onscroll="TableBuilder.scrollCellsTop(this, document.getElementById(\''+this.tableId+'_cells\'))">');
	sbResult.append('<div style="height:1px;margin-top:-1px;_margin-top:-2px;width:1px;font-size:0;line-height:0;" class="list_left_border"></div>');
	...
	sbResult.append('<div style="width:100%;overflow-y:hidden; overflow-x:hidden" id="'+ this.tableId +'_float" class="list_topdiv_header"');
	//添加标题拖拉事件
	sbResult.append(' onmousedown="TableBuilder.drag(event,\''+this.tableId+'\')"');
	sbResult.append('>');
	sbResult.append('<div style="overflow-y:scroll;overflow-x:hidden;width:100%;height:3000px;"  id="'+ this.tableId +'_float_div">');
	sbResult.append('<table border="0" cellspacing="0" cellpadding="0">');
	//输出合并标题
	...
	sbResult.append('</div>');
	//创建活动数据列
	sbResult.append('<div id="'+this.tableId+'_cells" style="  width:100%;height:'+ this.floatHeight +'px;overflow-x:scroll;overflow-y:scroll;" onscroll="TableBuilder.scrollCellsTop(this, document.getElementById(\''+this.tableId+'_static\').firstChild);document.getElementById(\''+ this.tableId +'_float_div\').scrollLeft=this.scrollLeft">');
	sbResult.append('<div style="height:1px;margin-top:-1px;_margin-top:-2px;font-size:0;line-height:0;"></div>');
	sbResult.append('<table border="0" cellspacing="0" cellpadding="0">');
...
TableBuilder.TRMouseOut = function(index,objleft,objright,visitedColor,evt){
	...
	if(objright){//选中右边的情况
		...
		TableBuilder.displayColor(objright, showColor)
		
		//获得冻结区域对应的行对象
		var objLeft = document.getElementById("TR_Left_" + objright.getAttribute("tableId") + "_" + index);
		TableBuilder.displayColor(objLeft, showColor);
	}
};
//表格鼠标划过效果
TableBuilder.TRMouseOver = function(index,objleft,objright,visitedColor,evt){
	...
	if(objright){//选中右边的情况
		...
		if(index != TableBuilder.iCurrentRow[tableId]){
			TableBuilder.displayColor(objright, objright.getAttribute("lightColor"))
			//获得冻结区域对应的行对象
			var objLeft = document.getElementById("TR_Left_" + tableId+ "_" + index);
			TableBuilder.displayColor(objLeft, objright.getAttribute("lightColor"));
		}
	}
};
...
TableBuilder.displaySelectedRowsColor= function(tableId,dataSize,visitedColor){
	...
	for(var i=0;i<dataSize;i++){
		...
		TableBuilder.displayColor(obj, showColor);
		//获得冻结区域对应的行对象
		var ol = document.getElementById("TR_Left_" + tableId + "_" + i);
		TableBuilder.displayColor(ol, showColor)
	}
};
//给当前点中行赋颜色
...
TableBuilder.displaySelectedRows = function(tableId){
	...
	if(count!=objs.length) {
		document.getElementsByName("CheckAll_myiframe0")[0].checked = false;
	}
...
TableBuilder.convertJS2HTML = function(str,checkFormat,convert){
	...
	if((convert==undefined || convert==true) && typeof(str)=='string'){
		str = str.replace(/&/g,"&amp;");
		str = str.replace(/"/g,"&quot;");
		str = str.replace(/</g,"&lt;");
		str = str.replace(/>/g,"&gt;");
	...
	//格式化显示
   	if(checkFormat){
   		...
   		if(checkFormat==5){
   			...
   			str = FormatKNumber(str/10000,TableFactory.WanYuanScale.length);
   		}else if(checkFormat>10){
   			str = FormatKNumber(str,checkFormat-10);
   		}
   	}
...
TableBuilder.scrollCellsTop = function(div1, div2){
	this.n = this.n ? this.n : 0;
	if(isIEBrowser() || this.n++ % 2 == 0){
	...
	if(inputType == 'select'){
		...
		if(inputRange){
			for(var i=0;i<inputRange.length;i+=2){
				...
				result.append('<label><input onfocus="clearErrors(this)" '+sHtmlStyle+' id="'+inputId+'_'+ i +'" name="'+inputname+'" type="'+ inputType +'" value="'+ inputRange[i] +'" '+sel+' onclick="tableDatas[\''+ tableId +'\']['+ row +']['+ col +']=this.value">' + inputRange[i+1])+'</label>';
			}
			if(row==0)
		...
		for(var i=0;i<inputRange.length;i+=2){
			...
			result.append('<label><input onfocus="clearErrors(this)" '+sHtmlStyle+' id="'+inputId+'_'+ i +'" name="'+inputname+'" type="'+ inputType +'" value="'+ inputRange[i] +'" '+sel+' onclick="tableDatas[\''+ tableId +'\']['+ row +']['+ col +']=TableBuilder.getCheckBoxValue(\''+inputname+'\')">' + inputRange[i+1])+'</label>';
		}
		...
		var iDZColIndex = TableFactory.getDZColIndexFromTalbe(tableId,col);
		result.append('<input onfocus="clearErrors(this)" '+sHtmlStyle+' class="list_all_input_1" id="'+inputId+'" name="'+inputname+'" type="text" value="'+ inputValue +'" onclick="AsDialog.PopView(\''+ sDWControlPath +'/TextAreaDialog.jsp\',\'detail=\'+this.value,\'dialogWidth:500px;dialogHeight:400px\', function(sReturnValue){var sColName = DZ['+tableIndex+'][1]['+iDZColIndex+'][15];setItemValue('+tableIndex+','+row+',sColName,sReturnValue);})">');
	}else if(inputType=='date'){
		var iDZColIndex = TableFactory.getDZColIndexFromTalbe(tableId,col);
		...
		if(inputValue) inputValue = inputValue.replace(/\"/g, "&quot;");
