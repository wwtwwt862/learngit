 ...
 * @since 2023-02-16
 */
public class DatacopyHandleServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException {
    	...
    	compClientID=ParameterUtil.getSafeParamter(compClientID);//解决安全扫描问题：进行参数验证
    	int a = 1;
        ...
        try {
        	...
        	try {
				...
				request.getRequestDispatcher("/PIDR/DatacopyManage/CodeTableImport.jsp?CloseFlag="+a+"&CompClientID=" + compClientID).forward(request, response);
				return;
			}
        	if(PIDRConstants.IMPORTRESULT_1 == a) {
				...
				DatacopyExcel dExcel = new DatacopyExcel();
				if (saveFilename.indexOf(".xls") < 0&&saveFilename.indexOf(".xlsx") < 0) {
					ARE.getLog().debug("获取的文件为非excel正确格式");
					a =  PIDRConstants.IMPORTRESULT_3;
					request.getRequestDispatcher("/PIDR/DatacopyManage/CodeTableImport.jsp?CloseFlag="+a+"&CompClientID=" + compClientID).forward(request, response);
					return;
				} 
				String resultString = dExcel.getListByExcel(saveFilename,excelServerPath);
				String[] resultStr = new String[5];
				int result = PIDRConstants.IMPORTRESULT_0;
				if (resultString.length() != 1) {
					resultStr = resultString.split("@");
					result = Integer.parseInt(resultStr[0]);
				}else {
				...
				if (result==PIDRConstants.IMPORTRESULT_4) {
					...
					request.getRequestDispatcher("/PIDR/DatacopyManage/CodeTableImport.jsp?CloseFlag=" + a + "&CompClientID=" + compClientID)
							.forward(request, response);
					return;
				}else if (result == PIDRConstants.IMPORTRESULT_0) {
					ARE.getLog().debug("存在主键冲突数据!");
					a =  PIDRConstants.IMPORTRESULT_0;
					request.getRequestDispatcher("/PIDR/DatacopyManage/CodeTableImport.jsp?CloseFlag="+a+"&CompClientID=" + compClientID).forward(request, response);
					return;
				}
				request.getRequestDispatcher("/PIDR/DatacopyManage/CodeTableImport.jsp?CloseFlag="+a+"&CompClientID=" + compClientID).forward(request, response);
