<%@ page contentType="text/html; charset=UTF-8"%><%@ include file="/IncludeBegin.jsp"%>
 <%--  
    ...
    Description:企业评级信息  历史数据    现金流量表   列表向详情的分发中转页面
--%>
<%
...
/*~[Describe=打开左侧本期数据详情,InputParam=无,OutputParam=无]~*/
 	function openLeft(){
		AsControl.OpenView("/PIDR/IntegatedQueryManage/HistoryDataManage/RateManage/HPidrRateCashflow/HPidrRateCashflowInfo.jsp","CustomerId=<%=customerId%>&FinstaYear=<%=finstaYear%>&SheetType=<%=sheetType%>&SheetTypeDivide=<%=sheetTypeDivide%>&SessionId=<%=sessionId%>&Stage=1&RightType=<%=rightType%>&Flag=<%=flag%>&fNo=<%=fNo%>&MessageType=<%=messageType%>","frameleft","");
	} 
    /*~[Describe=打开右侧上期数据详情,InputParam=无,OutputParam=无]~*/
	function openRight(){
		AsControl.OpenView("/PIDR/IntegatedQueryManage/HistoryDataManage/RateManage/HPidrRateCashflow/HPidrRateCashflowOldInfo.jsp","CustomerId=<%=customerId%>&FinstaYear=<%=finstaYear%>&SheetType=<%=sheetType%>&SheetTypeDivide=<%=sheetTypeDivide%>&SessionId=<%=sessionId%>&Stage=2&RightType=<%=rightType%>&Flag=<%=flag%>&fNo=<%=fNo%>&MessageType=<%=messageType%>","frameright","");
	}
