...
/*~[Describe=打开左侧本期数据详情,InputParam=无,OutputParam=无]~*/
 	function openLeft(){
		AsControl.OpenView("/PIDR/IntegatedQueryManage/CurrentDataManage/RateManage/PidrRateCashflow/PidrRateCashflowInfo.jsp","CustomerId=<%=customerId%>&FinstaYear=<%=finstaYear%>&SheetType=<%=sheetType%>&SheetTypeDivide=<%=sheetTypeDivide%>&Stage=1&OperationType=<%=operationType%>","frameleft","");
	} 
    /*~[Describe=打开右侧上期数据详情,InputParam=无,OutputParam=无]~*/
	function openRight(){
		AsControl.OpenView("/PIDR/IntegatedQueryManage/CurrentDataManage/RateManage/PidrRateCashflow/PidrRateCashflowOldInfo.jsp","CustomerId=<%=customerId%>&FinstaYear=<%=finstaYear%>&SheetType=<%=sheetType%>&SheetTypeDivide=<%=sheetTypeDivide%>&Stage=2&OperationType=<%=operationType%>","frameright","");
	}
