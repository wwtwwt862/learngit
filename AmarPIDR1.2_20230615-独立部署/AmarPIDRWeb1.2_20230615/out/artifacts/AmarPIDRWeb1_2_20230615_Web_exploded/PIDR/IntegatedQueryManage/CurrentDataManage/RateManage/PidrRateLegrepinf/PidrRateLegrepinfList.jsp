...
<%--  
	...
	Description: 综合查询-当期数据-企业评级信息记录-主要负责人段信息表列表页面
--%>
<%	
...
<script type="text/javascript">
	...
	function add(){
		var url = "/PIDR/IntegatedQueryManage/CurrentDataManage/RateManage/PidrRateLegrepinf/PidrRateLegrepinfInfo.jsp";
	...
	function del(){
		...
		layer.confirm("删除",{
			...
			if(recordFlag != '<%=PIDRConstants.RECORDFLAG_A%>' ){
				...
				var retValue = RunJavaMethodTrans("com.amarsoft.app.pidr.alldlt.PidrCascadeDelete", "deleteRateById", "customerId=" + customerId +",tableName="+'<%=PIDRConstants.TABLE_PIDR_RATE_LEGREPINF%>');
				alert(retValue);
				layer.close(index);
				reloadSelf();	
			}
	...
	function seeHis(){
		var url = "/PIDR/IntegatedQueryManage/HistoryDataManage/RateManage/HPidrRateLegrepinf/HPidrRateLegrepinfList.jsp";
		var customerId = getItemValue(0,getRow(0),'CustomerId');// 客户编号
	...
	function mdfcInfo(){
		...
		var retValue=RunJavaMethodTrans("com.amarsoft.app.pidr.mdfc.ModifyRateInfo","modifyRateInfo",
				"CustomerId=" + customerId + ",MdfcSgmtCode=C, ManagerOrgId=" + managerOrgId + ",CurUserId=<%=CurUser.getUserID()%>,CurOrgId=<%=CurOrg.getOrgID()%>");
		if(retValue!="操作成功") {
			alert(retValue);
			return;
		}
		var url = "/PIDR/ReportManage/ModifyDataManage/MdfcInfoManage/HpidrRateMdfcFrame.jsp";
		var params = 'SessionId=<%=PIDRConstants.SESSIONID_14%>&CustomerId=' + customerId + '&MdfcSgmtCode=C' ;
		AsDialog.PopView(url,params,'dialogWidth:1000px;dialogHeight:600px',function(){
			reloadSelf();	
		});
	}
